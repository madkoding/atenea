...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. Atenea's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-Atenea-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-Atenea-Owner") or "").strip()
...
from routes.background_routes import setup_background_routes
app.include_router(setup_font_routes())
app.include_router(setup_background_routes())
...
async def _startup_event():
    ...
    # `ATENEA_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("ATENEA_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (ATENEA_INPROCESS_TASKS=0); "
