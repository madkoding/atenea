#!/bin/bash
# Atenea — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything Atenea needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine Atenea
...
# Shell overrides (ATENEA_PORT / ATENEA_HOST) take top priority, then .env
# values (APP_PORT / APP_BIND), then built-in defaults.
PORT="${ATENEA_PORT:-${APP_PORT:-7860}}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
HOST="${ATENEA_HOST:-${APP_BIND:-127.0.0.1}}" # Set APP_BIND=0.0.0.0 in .env for LAN/Tailscale access.
PROBE_HOST="$HOST"
...
echo "▶ Atenea quick start for macOS"
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
    ...
    echo "    ATENEA_PORT=7900 ./start-macos.sh"
    exit 1
fi
...
if [ "$(uname -m)" = "arm64" ]; then
    ...
    cands="python3 python3.13 python3.12"
fi
...
#                  GGUF models on the GPU with no compile step
    #    - python@3.12 : installed only if no suitable (arm64) Python was found above
#
...
if [ "$MACHINE_ARCH" = "arm64" ]; then
    if command -v apfel >/dev/null 2>&1; then
        APFEL_LOG="${TMPDIR:-/tmp}/atenea-apfel.log"
...
# ATENEA_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$ATENEA_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
    (
        for _ in $(seq 1 90); do
            if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
                printf '\n'
                printf '  ┌────────────────────────────────────────────┐\n'
                printf '  │  ✓ Atenea is ready — opening your browser  │\n'
                printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting Atenea — it will open in your browser at $URL"
