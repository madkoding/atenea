#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for Atenea.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/Atenea.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/Atenea.dmg   — drag-to-Applications disk image (the downloadable).
#
...
# so rebuild if you move the repo. Override the port with ATENEA_PORT.
set -e

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_NAME="Atenea"
INSTALL_DIR="$REPO_DIR"
PORT="${ATENEA_PORT:-7860}"
DIST="$REPO_DIR/dist"
...
# ── Icon (best effort) — center-crop docs/atenea.jpg to a square .icns ──
if [ -f "$REPO_DIR/docs/atenea.jpg" ] && command -v sips >/dev/null 2>&1; then
  TMPIMG="$(mktemp -d)"
  ...
  sips -c 720 720 "$REPO_DIR/docs/atenea.jpg" --out "$TMPIMG/sq.png" >/dev/null 2>&1 || cp "$REPO_DIR/docs/atenea.jpg" "$TMPIMG/sq.png"
  sips -z 512 512 "$TMPIMG/sq.png" --out "$TMPIMG/icon.png" >/dev/null 2>&1
  if sips -s format icns "$TMPIMG/icon.png" --out "$APP/Contents/Resources/atenea.icns" >/dev/null 2>&1; then
    echo "  icon:        atenea.icns"
  else
    echo "  icon:        (skipped — conversion failed)"
  fi
  rm -rf "$TMPIMG"
else
  echo "  icon:        (skipped — no docs/atenea.jpg)"
fi
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.atenea.launcher</string>
    <key>CFBundleVersion</key>         <string>1.0</string>
    <key>CFBundleShortVersionString</key><string>1.0</string>
    <key>CFBundlePackageType</key>     <string>APPL</string>
    <key>CFBundleExecutable</key>      <string>$APP_NAME</string>
    <key>CFBundleIconFile</key>        <string>atenea</string>
...
#!/bin/bash
# Atenea.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/atenea-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"Atenea\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Atenea\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Atenea isn't set up yet. Open Terminal and run:

cd $INSTALL_DIR
python3.12 -m venv venv
./venv/bin/pip install -r requirements.txt
./venv/bin/python app_setup.py"
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Atenea failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Atenea is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag Atenea to Applications)"
