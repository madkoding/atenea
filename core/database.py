...
class EncryptedText(TypeDecorator):
    """Text column transparently encrypted at rest via src.security.secret_storage.
...
class Signature(TimestampMixin, Base):
    ...
    Fernet-encrypted at rest (see EncryptedText / src.security.secret_storage); a
...
def _migrate_chat_messages_fts():
    ...
    try:
        conn = sqlite3.connect(db_path)
        try:
            conn.execute("CREATE VIRTUAL TABLE IF NOT EXISTS temp._atenea_fts5_probe USING fts5(content)")
            conn.execute("DROP TABLE IF EXISTS temp._atenea_fts5_probe")
        except Exception as e:
