...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When Atenea runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends Atenea' own interpreter directory to PATH. If Atenea itself is
...
def _append_pip_install_runner_lines(runner_lines: list[str], cmd: str) -> None:
    ...
    runner_lines.append('  echo "[atenea] pip does not support --break-system-packages; installing without it."')
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'ATENEA_USER_SHELL="${SHELL:-}"',
        'if [ -n "$ATENEA_USER_SHELL" ] && [ -x "$ATENEA_USER_SHELL" ]; then',
        '  ATENEA_USER_PATH="$("$ATENEA_USER_SHELL" -ic \'printf "__ATENEA_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__ATENEA_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$ATENEA_USER_PATH" ]; then export PATH="$ATENEA_USER_PATH:$PATH"; fi',
        'fi',
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by Atenea.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$ATENEA_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $ATENEA_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$ATENEA_PREFLIGHT_EXIT"')
    runner_lines.append('fi')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for Atenea-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('ATENEA_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$ATENEA_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  ATENEA_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[atenea] vLLM CLI: $ATENEA_VLLM_BIN"')
    runner_lines.append('  ATENEA_VLLM_VERSION="$("$ATENEA_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$ATENEA_VLLM_VERSION" ]; then echo "[atenea] vLLM version: $ATENEA_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('ATENEA_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $ATENEA_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $ATENEA_CMD_EXIT ==="')
...
def _llama_cpp_rebuild_cmd() -> str:
    ...
    return (
        ...
        'echo "[atenea] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\atenea-sessions"
