...
def _mcp_oauth_base_dir() -> Path:
    """Directory that may contain OAuth files managed by Atenea."""
...
def setup_mcp_routes(mcp_manager: McpManager):
    ...
    async def oauth_callback(code: str, state: str, request: Request):
        ...
        require_admin(request)
        from src.clients.mcp_oauth import resolve_pending
        if resolve_pending(state, code):
            return HTMLResponse(_oauth_result_page(
                "Authorization Successful",
                "The MCP server is connecting. You can close this window and return to Atenea.",
                success=True,
            ))
...
<html><head>
<meta charset="UTF-8"><title>Authorize — Atenea</title>
<style>
