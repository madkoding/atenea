...
async def _auto_summarize_pass_single(days_back: int = 1, account_id: str | None = None, progress_cb=None) -> str:
    ...
    try:
        ...
        since = (_utcnow_naive() - _td(days=max(1, days_back))).strftime("%d-%b-%Y")
                        ...
                        if resp.ok:
                            ...
                            if summary:
                                ...
                                _c.execute("""
                                    ...
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                                """, (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid), _folder, subject, sender, summary, model, _utcnow_naive().isoformat()))
                                _c.commit()
                                _c.close()
                    ...
                    try:
                        ...
                        if reply:
                            ...
                            _c.execute("""
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?)
                            """, (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid), _folder, reply, model, _utcnow_naive().isoformat()))
                            _c.commit()
                            _c.close()
                        ...
                        if jm:
                            try:
                                ...
                                if isinstance(ops, list) and ops:
                                    from src.tools.implementations import do_manage_calendar
                                    for op in ops[:3]:
                    ...
                    try:
                        ...
                        _cc.execute(
                            ...
                            (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid),
                             _cal_run_count, _utcnow_naive().isoformat())
                        )
                        _cc.commit()
                        _cc.close()
                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Atenea (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Atenea" button
                                    import html as _h
                                    ...
                                    outer_alert["Date"] = _utcnow_naive().strftime("%a, %d %b %Y %H:%M:%S +0000")
                        ...
                        if not resp.ok:
                            ...
                            if parsed is not None:
                                ...
                                """, (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid), subject, sender,
                                      ...
                                      spam_reason, moved_to, model, _utcnow_naive().isoformat()))
                                _c.commit()
                                _c.close()
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "atenea_kind" if "atenea_kind" in cols else "'scheduled' AS atenea_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                atenea_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Atenea-Origin"] = "atenea-ui"
                outer["X-Atenea-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", atenea_kind or "scheduled")[:64]
                outer["X-Atenea-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `atenea-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `ATENEA_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `atenea-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("ATENEA_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `ATENEA_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (ATENEA_INPROCESS_POLLERS=0); "
            "drive `atenea-mail poll-scheduled` externally."
        )
        return
    import asyncio
