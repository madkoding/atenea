...
def _friendly_email_auth_error(protocol: str, host: str, error: object) -> str:
    ...
    if microsoft_basic_auth_failure:
        return (
            ...
            "Outlook/Office 365 IMAP/SMTP in most accounts. Atenea "
...
# Override at deploy time via ATENEA_MAIL_ATTACHMENTS_DIR. Defaults to a
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "atenea_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN atenea_kind TEXT")
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("ATENEA_IMAP_TIMEOUT_SECONDS"))
    ...
    try:
        ...
        if conn:
            try: conn.close()
                ...
                logger.exception("email_helpers: failed to close IMAP connection")
            try: conn.logout()
            except Exception:
                logger.exception("email_helpers: failed to logout IMAP connection")
        ...
        try:
            ...
            if ctx_conn:
                try: ctx_conn.logout()
                    ...
                    logger.exception("email_helpers: failed to logout IMAP context connection")
...
class SendEmailRequest(BaseModel):
    ...
    account_id: str | None = None
    # Internal marker for Atenea-generated mail (e.g. reminder, scheduled).
    atenea_kind: str | None = None
