...
def _apply_atenea_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-Atenea-Origin"] = ATENEA_MAIL_ORIGIN
    if kind:
        msg["X-Atenea-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-Atenea-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the Atenea marker header, but include the subject
                # fallback too. The fallback uses a distinct Atenea prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-Atenea-Kind "reminder" SUBJECT "Reminder (Atenea):"{from_clause})',
                )
                ...
                _since = (_utcnow_naive() - _td(days=30)).strftime("%d-%b-%Y")
                ...
                from datetime import timedelta as _td
                _before = (_utcnow_naive() - _td(days=30)).strftime("%d-%b-%Y")
    ...
    @router.delete("/atenea/reminders")
    async def delete_atenea_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as Atenea reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-Atenea-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (Atenea):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (Atenea):")})'))
                            # Legacy reminders created before the Atenea
            ...
            logger.error(f"delete_atenea_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", atenea_kind=None, atenea_ref=None,
    ):
        ...
        outer["Date"] = _utcnow_naive().strftime("%a, %d %b %Y %H:%M:%S +0000")
        _apply_atenea_headers(outer, atenea_kind or "scheduled", atenea_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, atenea_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("atenea_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = _utcnow_naive().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="atenea.local")
        ...
        if req.atenea_kind:
            _apply_atenea_headers(outer, req.atenea_kind)
    ...
    async def save_draft(req: SendEmailRequest, owner: str = Depends(require_owner)):
        ...
        msg["Date"] = _utcnow_naive().strftime("%a, %d %b %Y %H:%M:%S +0000")
            ...
            if mid:
                try:
                    ...
                    """, (
                        ...
                        subject, sender, content, model, _utcnow_naive().isoformat(),
                    ))
                    _c.commit()
                    _c.close()
                except Exception as e:
            ...
            if message_id:
                try:
                    ...
                    _c.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (message_id, owner, source_uid, source_folder, reply, model, _utcnow_naive().isoformat()))
                    _c.commit()
                    _c.close()
                except Exception as e:
