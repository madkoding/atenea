...
DOCKER_IN_CONTAINER_HINT = (
    "Not available inside the Atenea container by design. The image ships no "
...
def _package_pip_update_status(
    ...
    if pkg.get("kind") == "system" or not pkg.get("pip"):
        return PackageUpdateStatus(
            False, "Update this system dependency outside Atenea."
        )
    ...
    if name == "vllm" and binaries.get("vllm") and not dists.get("vllm"):
        return PackageUpdateStatus(
            False,
            "Using a vLLM CLI on PATH without Python package metadata; update it outside Atenea.",
        )
...
TMUX_LOG_DIR = Path(tempfile.gettempdir()) / "atenea-tmux"
...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f'ATENEA_USER_SHELL="${{SHELL:-}}"\n'
        f'if [ -n "$ATENEA_USER_SHELL" ] && [ -x "$ATENEA_USER_SHELL" ]; then\n'
        f'  ATENEA_USER_PATH="$("$ATENEA_USER_SHELL" -ic \'printf "__ATENEA_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__ATENEA_PATH__//p\' | tail -n 1 || true)"\n'
        f'  if [ -n "$ATENEA_USER_PATH" ]; then export PATH="$ATENEA_USER_PATH:$PATH"; fi\n'
        f"fi\n"
