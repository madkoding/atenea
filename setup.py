...
def create_default_admin():
    ...
    try:
        ...
        if sys.stdin.isatty() and not os.getenv("ATENEA_ADMIN_PASSWORD"):
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not os.getenv("ATENEA_ADMIN_PASSWORD"):
                print(f"        Temporary password: {password}")
                print(f"        ** Change it after first login. Set ATENEA_ADMIN_PASSWORD to choose your own. **")
        return "created"
