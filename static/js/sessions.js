...
let _sortMode = Storage.get('atenea-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Atenea Chat';
...
const FOLDER_STATE_KEY = 'atenea-folder-state';
const FOLDER_ORDER_KEY = 'atenea-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Atenea kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Atenea Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('atenea-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('atenea-session-sort', mode);
  else Storage.remove('atenea-session-sort');
  renderSessionList();
}
