...
const GROUP_STATE_KEY = 'atenea-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const storedGroupSessions = Storage.getJSON('atenea-group-sessions', []);
      const gids = Array.isArray(storedGroupSessions) ? storedGroupSessions : [];
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('atenea-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
