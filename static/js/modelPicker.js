...
const RECENT_KEY = 'atenea-model-recent';
const FAVORITES_KEY = 'atenea-model-favorites';
const RECENT_MAX = 5;
...
function _initModelPickerDropdown() {
  ...
  const _collapsedProviders = new Set(_loadList('atenea-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('atenea-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('atenea:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('atenea:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `atenea-model-favorites[0]`
