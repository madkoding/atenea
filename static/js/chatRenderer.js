...
const _ACTION_RECENTS_KEY = 'atenea-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'atenea-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Atenea';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    if (role !== 'user') {
      ...
      r.className = 'role';
      var _roleText = (isSlash || isCompacted) ? 'Atenea' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
