...
edited, or deleted in Atenea on a CalDAV-backed calendar only changed the local
...
def _stable_cal_id(remote_url: str, owner: str = "", account_id: str = "") -> str:
    ...
    from src.calendar_app.sync import _stable_cal_id as _sync_id
...
def build_event_ical(ev: dict) -> str:
    ...
    cal = Calendar()
    cal.add("prodid", "-//Atenea//CalDAV write-back//EN")
    cal.add("version", "2.0")
...
def _writeback_blocking(local_cal_id, ev, delete, url, username, password,
    ...
    from src.calendar_app.sync import _build_dav_client
