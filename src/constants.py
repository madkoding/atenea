...
DATA_DIR = os.getenv("ATENEA_DATA_DIR", os.path.join(BASE_DIR, "data"))

# Data file paths
# Single source of truth: every persisted file/dir lives under DATA_DIR, which
# is the ONLY place ATENEA_DATA_DIR is read. Import these constants instead of
...
MAIL_ATTACHMENTS_DIR = os.getenv("ATENEA_MAIL_ATTACHMENTS_DIR", os.path.join(DATA_DIR, "mail-attachments"))
...
def internal_api_base() -> str:
    ...
    running server over HTTP. Resolution order:
     ...
     failed" whenever the server is not on port 7001.
    """
    override = os.environ.get("ATENEA_INTERNAL_BASE")
    if override:
        return override.rstrip("/")
    return f"http://127.0.0.1:{os.environ.get('APP_PORT', '7001')}"
