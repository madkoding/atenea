...
class TaskScheduler:
    ...
    async def _note_pings_loop(self):
        ...
        from src.actions.builtin import action_ping_notes, TaskNoop
        while self._running:
    ...
    async def _event_pings_loop(self):
        ...
        from src.actions.builtin import action_ping_events, TaskNoop
        while self._running:
            owners = self._known_task_owners()
            for ow in (owners or [""]):
                try:
    ...
    async def _execute_task_locked(self, task_id: str, run_id: str, *, release_executing: bool = True):
        ...
        try:
            ...
            from src.actions.builtin import TaskDeferred, TaskNoop
    ...
    async def _execute_action(self, task, run_id: str | None = None) -> tuple:
        ...
        from src.actions.builtin import BUILTIN_ACTIONS

        action_fn = BUILTIN_ACTIONS.get(task.action)
        if not action_fn:
            return f"Unknown action: {task.action}", False

        from src.actions.builtin import TaskNoop
        try:
    ...
    async def _execute_llm_task(self, task, db) -> str:
        ...
        if crew and crew.enabled_tools:
            try:
                ...
                if isinstance(enabled, list) and enabled:
                    from src.tools.index import BUILTIN_TOOL_DESCRIPTIONS
        ...
        relevant_tools = None
        try:
            from src.tools.index import get_tool_index, ASSISTANT_ALWAYS_AVAILABLE
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            msg["X-Atenea-Kind"] = "task"
            msg["X-Atenea-Ref"] = str(task.id)
    ...
    async def _run_agent_loop(self, endpoint_url: str, model: str, task, session_id: str,
        ...
        try:
            ...
            from src.runtime.endpoint_resolver import normalize_base, build_headers
        ...
        try:
            from src.runtime.endpoint_resolver import resolve_utility_fallback_candidates
    ...
    async def _execute_research_task(self, task, db) -> str:
        ...
        from src.research.handler import RESEARCH_DATA_DIR, ResearchHandler
    ...
    async def _deliver_via_mcp(self, tool_name: str, task, result: str):
        ...
        args = {
            ...
            "body": result,
            "headers": {
                "X-Atenea-Origin": "atenea-ui",
                "X-Atenea-Kind": "task",
                "X-Atenea-Ref": str(task.id),
            },
        }
        if recipient:
