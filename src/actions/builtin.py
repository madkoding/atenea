...
async def action_tidy_sessions(owner: str, **kwargs) -> tuple[str, bool]:
    ...
    try:
        import asyncio
        from src.misc.session_actions import run_auto_sort
...
async def action_tidy_documents(owner: str, **kwargs) -> tuple[str, bool]:
    ...
    try:
        from src.actions.document import run_document_tidy
...
async def action_run_script(owner: str, script: str = "", host: str = "", **kwargs) -> tuple[str, bool]:
    ...
    target_host = (host or os.getenv("ATENEA_SCRIPT_HOST", "localhost")).strip()
            ...
            try:
                ...
                if newest is not None:
                    STATE_FILE.write_text(json.dumps({
                        ...
                        "last_run_at": _utcnow_naive().isoformat(),
                        "scanned": len(events),
                        "removed": len(removed),
        ...
        try:
            ...
            for i in range(0, len(llm_queue), BATCH):
                ...
                try:
                    ...
                    from src.misc.text_helpers import strip_think as _st
...
async def action_learn_sender_signatures(owner: str, **kwargs) -> tuple[str, bool]:
    ...
    try:
        ...
        def _pull_headers():
            ...
            try:
                ...
                try: conn.logout()
                    ...
                    logger.exception("builtin: failed to logout IMAP connection in _pull_headers")
            return results
        ...
        cutoff_iso = (_utcnow_naive() - _td(days=30)).isoformat()
        ...
        for addr, msgs in eligible[:20]:  # cost cap per run
            ...
            def _fetch_bodies(_msgs):
                ...
                try:
                    ...
                    try: conn2.logout()
                        ...
                        logger.exception("builtin: failed to logout IMAP connection in _fetch_bodies")
                return bodies
            ...
            try:
                ...
                conn.execute(
                    ...
                    "VALUES (?, ?, ?, ?, ?, ?)",
                    (addr, cached_sig, len(bodies), _utcnow_naive().isoformat(), model, "llm"),
                )
                conn.commit()
                conn.close()
                analyzed += 1
        ...
        try:
            ...
            try:
                ...
                try: conn.logout()
                    ...
                    logger.exception("builtin: failed to logout IMAP connection in daily_brief")
...
async def action_check_email_urgency(owner: str, **kwargs) -> tuple[str, bool]:
    ...
    try:
        ...
        from routes.email_helpers import _imap_connect, _decode_header
        from src.runtime.endpoint_resolver import resolve_endpoint, resolve_utility_fallback_candidates
        ...
        AGE_CUTOFF = _utcnow_naive() - _td(days=7)
        TRIAGE_VERSION = 3
        CATEGORY_TAGS = {
            ...
            def _scan_one(account=acc, cache_uids=cache.get("uids", {})):
                ...
                try:
                    ...
                    for uid_b in uids:
                        ...
                        try:
                            ...
                            if not raw:
                                continue
                            msg = _email_mod.message_from_bytes(raw)
                            # Skip Atenea-generated reminders so the scanner
                            ...
                            _ate_origin = (msg.get("X-Atenea-Origin") or "").strip().lower()
                            _ate_kind = (msg.get("X-Atenea-Kind") or "").strip().lower()
                            ...
                            if (_ate_origin == "atenea-ui" or _ate_kind == "reminder"
                                    or _raw_subj.startswith("reminder (atenea):")
                    ...
                    try: conn.logout()
                        ...
                        logger.exception("builtin: failed to logout IMAP connection in _scan_one")
                return results
                    ...
                    if _row:
                        ...
                        _conn.execute(
                            ...
                            (_msg_id, _owner_key, _uid_only, _v.get("subject", ""),
                             ...
                             _dt2.now(UTC).replace(tzinfo=None).isoformat()),
                        )
                _conn.commit()
            finally:
                _conn.close()
        ...
        if new_urgent:
            ...
            # straight into the Atenea Email tab).
...
BUILTIN_ACTION_INFO = {
    ...
    "run_script": "Run a script locally or on ATENEA_SCRIPT_HOST",
