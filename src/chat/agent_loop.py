...
Streaming agent loop for atenea-ui.
...
from src.tools.security import blocked_tools_for_owner, plan_mode_disabled_tools
from src.tools.policy import GUIDE_ONLY_DIRECTIVE, ToolPolicy
from src.agent.tools_facade import (
    parse_tool_blocks,
    strip_tool_blocks,
    execute_tool_block,
    format_tool_result,
...
def apply_language(prompt: str, lang: str | None) -> str:
...
```
GENERIC LOOPBACK to allowed Atenea internal endpoints. Use this whenever the user wants something the UI can do but there's NO named tool for it. Many UI buttons hit /api/* endpoints — you can hit allowed ones. Auth is handled automatically.
...
def _build_system_prompt(
    ...
    try:
        from src.misc.user_time import current_datetime_prompt
...
def _build_base_prompt(
    ...
    """
    from src.tools.index import ALWAYS_AVAILABLE
    ...
    if not suppress_local_context:
        from src.integrations.registry import get_integrations_prompt
...
async def stream_agent_loop(
    ...
    if not guide_only and not _relevant_tools:
        try:
            from src.tools.index import get_tool_index, ALWAYS_AVAILABLE
    ...
    if not guide_only and not _relevant_tools and _retrieval_query:
        from src.tools.index import ALWAYS_AVAILABLE, ToolIndex
        _relevant_tools = set(ALWAYS_AVAILABLE)
    ...
    _t3 = time.time()
    try:
        from src.chat.context_compactor import trim_for_context
        from src.chat.context_budget import compute_input_token_budget, DEFAULT_HARD_MAX
    ...
    if not _is_teacher_run and not guide_only:
        try:
            from src.agent.teacher_escalation import run_teacher_inline
