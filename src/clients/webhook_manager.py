...
class WebhookManager:
    ...
    async def deliver_test(self, webhook_id: str, url: str, encrypted_secret: str | None):
        ...
        await self._deliver(webhook_id, url, decrypted, "webhook.test", {"message": "Test ping from Atenea"})
    ...
    async def _deliver(self, webhook_id: str, url: str, secret: str | None, event: str, payload: dict):
        ...
        headers = {
            ...
            "X-Atenea-Event": event,
            "User-Agent": "Atenea-Webhook/1.0",
        }
        if secret:
            sig = hmac.new(secret.encode(), body.encode(), hashlib.sha256).hexdigest()
            headers["X-Atenea-Signature"] = sig
