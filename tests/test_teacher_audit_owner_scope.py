...
import src.agent.teacher_escalation as teacher_escalation
...
def test_call_teacher_scopes_model_resolution_to_owner(monkeypatch):
    ...
    monkeypatch.setattr("src.agent.ai_interaction._resolve_model", fake_resolve_model)
    monkeypatch.setattr("src.agent.ai_interaction._TEACHER_SYSTEM_PROMPT", "sys", raising=False)
...
def test_audit_teacher_resolution_scoped_to_owner(monkeypatch):
    ...
    monkeypatch.setattr("src.runtime.endpoint_resolver.resolve_endpoint", fake_resolve_endpoint)
