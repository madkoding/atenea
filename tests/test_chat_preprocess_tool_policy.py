...
async def test_preprocess_can_skip_external_context_and_attachment_work(monkeypatch):
    ...
    monkeypatch.setattr("src.chat.handler.extract_transcript_async", _fail_transcript)
    monkeypatch.setattr("src.chat.handler.fetch_youtube_comments", _fail_comments)
    monkeypatch.setattr(
        "src.chat.handler.model_supports_vision",
